...
const App: React.FC = () => {
  ...
  const handleSoanBai = async () => {
    ...
    if (!customPrompt) return alert("Vui lòng chọn hoặc nhập lệnh soạn thảo!");
    
    setLoading(true);
    setAiResponse(""); // Xóa kết quả cũ để hiển thị trạng thái đang soạn
    try {
      ...
      <header className="h-40 bg-emerald-700 px-10 flex justify-between items-center shrink-0 border-b-4 border-emerald-900 shadow-2xl">
        <div className="flex items-center gap-6">
          <div onClick={() => avatarInputRef.current?.click()} className="w-24 h-24 rounded-full border-4 border-white/30 overflow-hidden bg-emerald-800 flex items-center justify-center cursor-pointer shadow-xl">
             {avatarUrl ? <img src={avatarUrl} className="w-full h-full object-cover" /> : <span className="text-[10px] text-white font-black text-center uppercase">GV TÙNG</span>}
      ...
      <main className="flex-1 grid grid-cols-12 gap-8 p-8 overflow-hidden">
        <aside className="col-span-3 space-y-6 flex flex-col min-h-0">
          <div className="bg-[#1e293b] rounded-2xl p-6 border border-slate-700 shadow-xl space-y-4 shrink-0">
            <h2 className="text-[10px] font-black text-emerald-400 uppercase italic underline decoration-emerald-900">⚙️ Thiết lập bài dạy</h2>
            ...
            <div className="relative">
              ...
              {showPromptMenu && (
                <div className="absolute top-full left-0 w-full bg-slate-800 border border-slate-600 rounded-xl mt-2 overflow-hidden z-[60] shadow-2xl">
                  ...
                  <button onClick={() => {setCustomPrompt(getPrompt('ppt')); setShowPromptMenu(false);}} className="w-full text-left px-5 py-4 hover:bg-emerald-600 text-[10px] font-black uppercase text-white border-b border-slate-700">💻 Soạn Slide</button>
          ...
          <div className="bg-[#1e293b] rounded-2xl border border-slate-700 flex-1 flex flex-col min-h-0 overflow-hidden shadow-xl">
            ...
            <div className="p-4 flex-1 flex flex-col overflow-hidden">
              <div onClick={() => fileInputRef.current?.click()} className="h-20 shrink-0 border-2 border-dashed border-slate-600 rounded-xl flex flex-col items-center justify-center cursor-pointer hover:border-emerald-500 transition-all mb-4">
              ...
              <div className="flex-1 overflow-y-auto space-y-2 custom-scrollbar pr-1">
                {selectedFiles.map((file, idx) => (
                  <div key={idx} className="bg-slate-900 p-2 rounded-lg border border-slate-700 text-[10px] flex justify-between items-center italic">
                    <span className="truncate w-40 text-emerald-300">📄 {file.name}</span>
          ...
          <button onClick={handleSoanBai} disabled={loading} className="w-full py-7 rounded-2xl font-black text-sm uppercase bg-blue-600 hover:bg-blue-500 shadow-2xl transition-all">
        ...
        <div className="col-span-9 grid grid-cols-12 gap-8 h-full">
           <section className="col-span-4 flex flex-col min-h-0">
             <div className="bg-[#1e293b] rounded-2xl border border-slate-700 flex flex-col h-full shadow-2xl overflow-hidden">
                ...
                <textarea value={customPrompt} onChange={(e) => setCustomPrompt(e.target.value)} className="w-full flex-1 bg-transparent p-6 text-sm text-slate-100 outline-none resize-none custom-scrollbar leading-relaxed" />
             </div>
           </section>
           ...
           <section className="col-span-8 flex flex-col min-h-0">
             <div className="bg-[#1e293b] rounded-2xl border border-slate-700 flex flex-col h-full shadow-2xl relative overflow-hidden">
                <div className="px-10 py-5 bg-[#0f172a] border-b border-slate-700 flex justify-between items-center">
                  ...
                  <button onClick={() => saveAs(new Blob([aiResponse]), `SoanBai_${tenBai}.docx`)} className="px-6 py-3 bg-emerald-600 text-white rounded-xl text-[10px] font-black uppercase hover:bg-emerald-500">♻️ XUẤT HỒ SƠ</button>
                </div>
                
                {/* HIỂN THỊ TRẠNG THÁI ĐANG SOẠN NGAY TRÊN THẺ PREVIEW */}
                <div className="flex-1 p-12 overflow-y-auto text-xl leading-[1.8] text-slate-300 whitespace-pre-wrap font-medium custom-scrollbar">
                   {loading ? (
                     <div className="h-full flex flex-col items-center justify-center space-y-6 text-orange-400">
                        <div className="w-16 h-16 border-4 border-orange-500 border-t-transparent rounded-full animate-spin"></div>
                        <div className="text-center">
                           <p className="font-black text-2xl animate-pulse">ĐANG SOẠN BÀI...</p>
                           <p className="text-sm italic text-slate-500 mt-2 uppercase tracking-widest">Hệ thống đang kiến tạo bài dạy: {tenBai || "Chưa đặt tên"}</p>
                        </div>
                     </div>
                   ) : (
                     aiResponse || <div className="text-slate-600 italic text-center mt-20 uppercase text-sm tracking-tighter">Sẵn sàng thực thi. Vui lòng nhấn nút "Kích hoạt"</div>
                   )}
                </div>
             </div>
           </section>
        ...
        <div className="fixed inset-0 bg-black/85 backdrop-blur-md z-[2000] flex items-center justify-center p-4">
           <div className="bg-[#020817] w-full max-w-2xl h-[75vh] rounded-[2rem] border border-emerald-500/30 flex flex-col overflow-hidden shadow-2xl">
              <div className="p-6 bg-emerald-800 flex justify-between items-center text-white">
                 <span className="font-black text-xs uppercase italic">🤖 Trợ lý tư vấn sư phạm</span>
                 <button onClick={() => setIsAssistantOpen(false)} className="px-4 py-2 bg-white/10 rounded-full font-black text-[9px] uppercase">✕ ĐÓNG</button>
              </div>
              <div className="flex-1 p-6 overflow-y-auto space-y-4 custom-scrollbar bg-slate-900/50">
                 {chatHistory.map((m, i) => (
                   <div key={i} className={m.role === 'user' ? 'text-right' : 'text-left'}>
                      <div className={`inline-block p-4 rounded-2xl text-sm max-w-[85%] ${m.role === 'user' ? 'bg-blue-600 text-white shadow-lg' : 'bg-slate-800 text-slate-300 border border-slate-700'}`}>
                         {m.text}
                      </div>
                   </div>
                 ))}
              ...
              <div className="p-6 border-t border-slate-800 flex gap-2">
                 <input value={chatInput} onChange={(e) => setChatInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && setChatHistory([...chatHistory, {role: 'user', text: chatInput}])} className="flex-1 bg-slate-900 border border-slate-700 rounded-xl px-4 text-xs text-white outline-none" placeholder="Hỏi trợ lý..." />
                 <button className="bg-emerald-600 px-6 py-3 rounded-xl font-black text-[10px] text-white">GỬI</button>
              </div>
           </div>
        </div>
      )}

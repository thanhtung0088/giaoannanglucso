...
const App: React.FC = () => {
  ...
  const handleAiAction = async () => {
    ...
    if (!apiKey) return alert("Thầy Tùng hãy cấu hình API Key!");
    setLoading(true);
    setIsChatOpen(true);
    try {
      const genAI = new GoogleGenerativeAI(apiKey);
      // ĐỂ TRÁNH LỖI 404, THẦY CÓ THỂ THAY ĐỔI TÊN MODEL CHÍNH XÁC TẠI ĐÂY:
      // Các mã khả thi: "gemini-3.0-flash", "gemini-3.0-pro-exp", hoặc "gemini-2.5-flash-lite"
      ...
      // THÔNG BÁO LỖI MỚI - KHÔNG KHUYÊN DÙNG BẢN CŨ
      setAiResponse(`❌ HỆ THỐNG HIỆN TẠI CHƯA PHẢN HỒI MODEL 3.0: \n\nChi tiết: ${error.message}\n\nThầy Tùng hãy kiểm tra lại quyền truy cập (Whitelist) của API Key đối với các dòng Next-Gen trên Google AI Studio.`);
  ...
  return (
    <div className="h-screen bg-[#020617] text-slate-200 font-sans overflow-hidden flex flex-col p-4">
      <header className="h-20 mb-4 px-10 flex justify-between items-center bg-slate-900/80 rounded-2xl border border-blue-500/30 shadow-[0_0_30px_rgba(30,58,138,0.5)] shrink-0">
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-4 border-r border-slate-700 pr-8">
            <div className="w-12 h-12 bg-gradient-to-br from-blue-500 to-indigo-700 rounded-xl flex items-center justify-center text-white text-2xl font-black shadow-lg">⚡</div>
            <div>
          ...
          <div className="bg-orange-600 text-white px-8 py-2 rounded-full font-black italic text-sm shadow-orange-900/20 shadow-xl">HỆ THỐNG TRỢ LÝ THẾ HỆ 3.0</div>
        </div>
        <div className="text-right flex flex-col items-end">
          <span className="text-[11px] font-black text-blue-400 bg-blue-500/10 px-4 py-1 rounded-md border border-blue-500/30">STATUS: NEXT-GEN READY</span>
        </div>
      </header>
      ...
      <main className="flex-1 grid grid-cols-12 gap-6 overflow-hidden">
        <div className="col-span-3 flex flex-col gap-5 overflow-hidden">
          <div className="bg-slate-900/60 p-6 rounded-2xl border border-slate-800 shadow-xl space-y-5">
            <h2 className="text-[10px] font-black uppercase text-blue-500 tracking-widest">⚙️ Thiết lập chuyên môn</h2>
            ...
            <button onClick={() => setShowPromptMenu(!showPromptMenu)} className="w-full py-4 bg-orange-600 text-white rounded-xl font-black text-[10px] uppercase shadow-lg flex justify-between px-5 items-center hover:bg-orange-500 transition-all">
               📝 Thư viện lệnh Prompt mẫu {showPromptMenu ? '▲' : '▼'}
            </button>
            {showPromptMenu && (
              <div className="absolute left-10 w-[280px] mt-2 bg-slate-900 rounded-xl shadow-2xl border border-blue-500/20 overflow-hidden z-[500]">
                {promptsMau.map((p, i) => (
                  <button key={i} onClick={() => { setCustomPrompt(p.content); setShowPromptMenu(false); }} className="w-full px-5 py-4 text-left text-[11px] font-bold hover:bg-blue-900 border-b border-slate-800 text-slate-300">
                    {p.label}
                  </button>
                ))}
              </div>
          ...
          {/* HIỂN THỊ FILE - ĐẢM BẢO HIỂN THỊ ĐỦ VÀ ĐẾM ĐÚNG */}
          <div className="bg-slate-900/60 p-6 rounded-2xl border border-slate-800 shadow-xl flex-1 flex flex-col min-h-0">
            ...
            <div onClick={() => tailieuRef.current?.click()} className="py-8 border-2 border-dashed border-slate-800 rounded-2xl flex flex-col items-center justify-center cursor-pointer hover:bg-blue-900/20 hover:border-blue-500/50 transition-all shrink-0 mb-4 group">
              <span className="text-3xl mb-2 group-hover:scale-110 transition-transform">📎</span>
              <p className="text-[9px] font-black text-slate-500 uppercase">Gắn tối thiểu 4 tài liệu</p>
              <input type="file" ref={tailieuRef} className="hidden" multiple onChange={handleFileChange} />
            </div>

            <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-2">
              {selectedFiles.map((file, idx) => (
                <div key={idx} className="flex items-center gap-3 p-3 bg-black/40 rounded-xl border border-slate-800 hover:border-blue-500/30 transition-all group">
                  <span className="text-lg">📄</span>
                  <div className="flex-1 min-w-0">
                    <p className="text-[10px] font-black truncate text-slate-400 group-hover:text-blue-400">{file.name}</p>
                    <p className="text-[8px] text-slate-600 font-bold uppercase">Ready for Analysis</p>
                  </div>
                  <button onClick={(e) => { e.stopPropagation(); setSelectedFiles(prev => prev.filter((_, i) => i !== idx)) }} className="text-red-900 font-black px-2 hover:text-red-500 transition-colors">✕</button>
                </div>
              ))}
          ...
          <button onClick={handleAiAction} disabled={loading} className="w-full py-5 bg-gradient-to-r from-blue-700 to-indigo-800 text-white rounded-2xl font-black text-xs uppercase shadow-2xl hover:brightness-125 transition-all active:scale-95">
             {loading ? "⚡ ĐANG PHÂN TÍCH..." : "🚀 KÍCH HOẠT GEMINI 3.0"}
          </button>
        </div>
          ...
          <div className="bg-slate-900/40 backdrop-blur-3xl rounded-[2.5rem] border border-slate-800 flex flex-col flex-1 shadow-2xl relative overflow-hidden">
            <div className="px-10 py-6 border-b border-slate-800 flex justify-between items-center bg-black/20">
              <div className="flex gap-4">
                 <button className="px-8 py-2 bg-blue-600/10 text-blue-400 border border-blue-500/20 rounded-lg text-[10px] font-black uppercase">Chế độ chuyên sâu</button>
              </div>
              <button onClick={() => setCustomPrompt("")} className="text-[10px] font-black text-slate-600 hover:text-white uppercase">Xóa trắng vùng soạn thảo</button>
            </div>
            <textarea value={customPrompt} onChange={(e)=>setCustomPrompt(e.target.value)} placeholder="Nhập yêu cầu sư phạm tại đây..." className="w-full flex-1 bg-transparent p-12 text-lg outline-none resize-none font-medium text-slate-300 leading-relaxed custom-scrollbar" />
            <div className="absolute bottom-10 right-10 flex gap-4">
                <button onClick={() => window.open('https://canva.com', '_blank')} className="px-10 py-5 bg-[#8b3dff] text-white rounded-2xl text-[11px] font-black uppercase shadow-2xl hover:scale-105 transition-all">🎨 Mở Canva</button>
                <button onClick={() => {saveAs(new Blob([aiResponse]), "HoSo_ThayTung_3.0.docx");}} className="px-10 py-5 bg-emerald-600 text-white rounded-2xl text-[11px] font-black uppercase shadow-2xl hover:bg-emerald-500 transition-all">📥 Xuất file hồ sơ</button>
            </div>
          </div>
        </div>
      </main>

      {/* MODAL KẾT QUẢ NEXT-GEN */}
      <div className={`fixed bottom-10 right-10 z-[300] transition-all duration-700 transform ${isChatOpen ? 'w-[94vw] md:w-[850px] opacity-100 translate-y-0 scale-100' : 'w-0 opacity-0 translate-y-20 scale-90 pointer-events-none'}`}>
          <div className="bg-[#020617] rounded-[3.5rem] border border-blue-500/20 shadow-[0_0_100px_rgba(30,58,138,0.3)] flex flex-col h-[80vh] overflow-hidden">
            <div className="p-10 bg-gradient-to-r from-blue-900 to-black text-white flex justify-between items-center border-b border-blue-500/10">
                <div className="flex items-center gap-6">
                  <div className="w-12 h-12 bg-blue-600/20 rounded-full flex items-center justify-center animate-spin-slow shadow-[0_0_15px_rgba(37,99,235,0.5)]">💎</div>
                  <div>
                    <span className="text-[13px] font-black uppercase tracking-[0.4em] block text-blue-400">Next-Gen Intelligence</span>
                    <span className="text-[9px] text-slate-500 font-bold uppercase tracking-widest italic">Biên soạn bởi trí tuệ nhân tạo Gemini 3 Series</span>
                  </div>
                </div>
            ...
            <div className="p-16 overflow-y-auto text-[19px] leading-[1.9] whitespace-pre-wrap font-medium text-slate-300 flex-1 custom-scrollbar selection:bg-blue-500/30">
                {loading ? "⚡ HỆ THỐNG ĐANG KHỞI TẠO DỮ LIỆU TỪ MÔ HÌNH THẾ HỆ 3..." : aiResponse || "Sẵn sàng."}
            </div>
          </div>
      </div>
    </div>

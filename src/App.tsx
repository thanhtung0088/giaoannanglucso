...
const App: React.FC = () => {
  ...
  const [avatarUrl, setAvatarUrl] = useState<string | null>(localStorage.getItem("permanent_logo_v79"));
  ...
  const handleAvatarChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files?.[0]) {
      ...
      reader.onload = (event) => {
        ...
        setAvatarUrl(base64);
        localStorage.setItem("permanent_logo_v79", base64);
      };
  ...
  const getHardcodedPrompt = (type: string) => {
    ...
    if (type === '5512') return `Bạn là chuyên gia xây dựng Kế hoạch bài dạy theo Chương trình GDPT 2018. Hãy soạn KẾ HOẠCH BÀI DẠY (KHBD) theo Công văn 5512/BGDĐT-GDTrH, Phụ lục 4 ${thongTin}. TÍCH HỢP: Năng lực số, Quyền con người, QP-AN, Tư tưởng Hồ Chí Minh.`;
    if (type === 'ppt') return `Soạn nội dung PowerPoint 10 slide cho bài dạy: ${thongTin}.`;
    if (type === '7991') return `Soạn đề kiểm tra ma trận, đặc tả theo CV 7991 cho: ${thongTin}.`;
    if (type === 'ontap') return `Soạn đề cương ôn tập trọng tâm cho: ${thongTin}.`;
    return "";
  };
  ...
  const handleSoanBai = async () => {
    ...
    try {
      ...
      // SỬ DỤNG MODEL GEMINI 3 FLASH THEO THÔNG BÁO 2026
    ...
    <div className="h-screen bg-slate-600 text-slate-100 overflow-hidden flex flex-col font-sans italic">
      <header className="h-32 bg-emerald-700 px-10 flex justify-between items-center shrink-0 border-b-4 border-emerald-900 shadow-2xl z-50">
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            <p className="text-xs font-bold text-emerald-200 uppercase mt-2">GV: NGUYỄN THANH TÙNG</p>
          </div>
        </div>
      ...
      <main className="flex-1 grid grid-cols-12 gap-6 p-6 overflow-hidden">
        <aside className="col-span-3 space-y-6 flex flex-col min-h-0">
          <div className="bg-[#1e293b] rounded-3xl p-6 border border-slate-500 shadow-2xl space-y-4 shrink-0">
            <h2 className="text-xs font-black text-emerald-400 uppercase">⚙️ Thiết lập bài dạy</h2>
            <select value={monHoc} onChange={(e)=>setMonHoc(e.target.value)} className="w-full bg-slate-900 border border-slate-600 rounded-xl p-4 text-sm font-bold text-white uppercase outline-none">
              {dsMonHoc.map(m => <option key={m} value={m}>{m}</option>)}
            </select>
            <select value={khoiLop} onChange={(e)=>setKhoiLop(e.target.value)} className="w-full bg-slate-900 border border-slate-600 rounded-xl p-4 text-sm font-bold text-white uppercase outline-none">
              {dsKhoi.map(k => <option key={k} value={k}>{k}</option>)}
            </select>
            <input type="text" value={tenBai} onChange={(e)=>setTenBai(e.target.value)} className="w-full bg-slate-900 border border-slate-600 rounded-xl p-4 text-sm font-bold text-white outline-none" placeholder="Nhập tên bài dạy..." />
            <div className="grid grid-cols-2 gap-3">
              <input type="text" value={soTiet} onChange={(e)=>setSoTiet(e.target.value)} className="bg-slate-900 border border-slate-600 rounded-xl p-4 text-sm font-bold text-white outline-none" placeholder="1" />
            ...
            {showPromptMenu && (
              <div className="absolute left-10 w-64 bg-slate-800 border-2 border-slate-500 rounded-2xl z-[100] shadow-2xl font-black italic">
                <button onClick={() => {setCustomPrompt(getHardcodedPrompt('5512')); setShowPromptMenu(false);}} className="w-full text-left px-6 py-4 hover:bg-emerald-600 text-white border-b border-slate-700 text-[10px] uppercase">📑 SOẠN KHBD CV 5512</button>
                <button onClick={() => {setCustomPrompt(getHardcodedPrompt('ppt')); setShowPromptMenu(false);}} className="w-full text-left px-6 py-4 hover:bg-emerald-600 text-white border-b border-slate-700 text-[10px] uppercase">💻 BÀI GIẢNG PPT</button>
                <button onClick={() => {setCustomPrompt(getHardcodedPrompt('7991')); setShowPromptMenu(false);}} className="w-full text-left px-6 py-4 hover:bg-emerald-600 text-white border-b border-slate-700 text-[10px] uppercase">✍️ ĐỀ KIỂM TRA 7991</button>
                <button onClick={() => {setCustomPrompt(getHardcodedPrompt('ontap')); setShowPromptMenu(false);}} className="w-full text-left px-6 py-4 hover:bg-emerald-600 text-white text-[10px] uppercase">📚 ĐỀ CƯƠNG ÔN TẬP</button>
              </div>
            )}
          </div>
          ...
          <div className="bg-[#1e293b] rounded-3xl border border-slate-500 flex flex-col flex-1 overflow-hidden shadow-2xl shrink-0 min-h-[350px]">
            <div className="bg-slate-900 px-6 py-4 border-b border-slate-700 text-emerald-400 font-black text-xs uppercase underline">📁 HỒ SƠ TÀI LIỆU (+)</div>
            <div className="p-4 flex flex-col h-full bg-slate-800/40">
              ...
              <div className="flex-1 overflow-y-auto space-y-2 pr-1 custom-scrollbar">
                {selectedFiles.map((f, i) => (
                  <div key={i} className="bg-slate-900 p-4 rounded-xl border border-slate-700 text-[11px] flex justify-between items-center italic animate-in slide-in-from-left-2">
                    <span className="truncate w-40 text-emerald-300 font-black">📄 {f.name}</span>
                    <button onClick={() => setSelectedFiles(prev => prev.filter((_, idx) => idx !== i))} className="text-red-500 font-black px-2 hover:scale-125">✕</button>
                  </div>
                ))}
                {selectedFiles.length === 0 && <div className="h-full flex flex-col items-center justify-center text-[10px] text-slate-500 font-black opacity-30">Tối đa 5 file...</div>}
              </div>
            </div>
          </div>
        ...
        <section className="col-span-3 flex flex-col min-h-0">
          <div className="bg-[#1e293b] rounded-3xl border border-slate-500 flex flex-col h-full shadow-2xl overflow-hidden">
             <div className="px-6 py-4 bg-slate-900 border-b border-slate-700 text-[10px] font-black text-orange-500 uppercase">Thẻ Workspace</div>
        ...
        <section className="col-span-6 flex flex-col min-h-0 relative">
          <div className="bg-[#1e293b] rounded-3xl border border-slate-500 flex flex-col h-full shadow-2xl overflow-hidden">
             <div className="px-10 py-5 bg-slate-900 border-b border-slate-700 flex justify-between items-center shrink-0">
               <span className="text-xs font-black text-emerald-500 uppercase underline underline-offset-8">Bảng Preview Kết Quả AI</span>
             ...
             <div className="flex-1 bg-black/10 p-12 overflow-y-auto custom-scrollbar">
                {loading ? (
                  ...
                  <div className="text-xl leading-loose text-slate-100 whitespace-pre-wrap font-medium">{aiResponse || "Sẵn sàng hỗ trợ Thầy Tùng..."}</div>
                )}
             </div>
          </div>
        </section>
      ...
      {isChatOpen && (
        <div className="fixed bottom-32 right-10 w-96 h-[500px] bg-slate-800 border-4 border-emerald-600 rounded-3xl shadow-2xl z-[2000] flex flex-col overflow-hidden">
           <div className="bg-emerald-600 p-4 flex justify-between items-center text-white font-black uppercase text-[10px]"><span>Trợ lý AI Thầy Tùng</span><button onClick={() => setIsChatOpen(false)}>✕</button></div>
           ...
           <div className="p-4 bg-slate-800 flex gap-2 border-t border-slate-700">
              ...
              <button onClick={handleAssistantChat} className="bg-emerald-600 px-4 rounded-xl text-white font-black text-xs hover:bg-emerald-500">GỬI</button>
           </div>
        </div>
      )}

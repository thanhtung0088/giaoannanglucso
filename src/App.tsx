...
const App: React.FC = () => {
  ...
  const handleAiAction = async () => {
    ...
    if (!apiKey) return alert("Thầy vui lòng cấu hình API Key mới!");
    setLoading(true);
    setIsChatOpen(true);
    try {
      ...
      setAiResponse(`⚠️ Lỗi: ${error.message}\n\nThầy Tùng hãy thử đổi sang API Key của Gmail khác nếu vẫn gặp lỗi này nhé.`);
  ...
  return (
    <div className="h-screen bg-[#5a9ad4] text-slate-800 font-sans overflow-hidden flex flex-col p-3">
      <header className="h-20 mb-3 px-8 flex justify-between items-center bg-white/95 rounded-xl border border-white shadow-xl shrink-0">
        ...
        <div className="bg-blue-600 text-white px-5 py-2 rounded-lg font-black text-[10px] uppercase shadow-md">v27.0 - CHUẨN HÓA</div>
      </header>
      ...
      <main className="flex-1 grid grid-cols-12 gap-4 overflow-hidden">
        <div className="col-span-3 flex flex-col gap-4 overflow-hidden">
          <div className="bg-white/95 p-5 rounded-xl border border-white shadow-lg space-y-4">
            <h2 className="text-[10px] font-black uppercase text-blue-600">⚙️ Thiết lập nhanh</h2>
            ...
            <div className="relative">
              ...
              {showPromptMenu && (
                <div className="absolute top-full left-0 w-full mt-1 bg-white rounded-lg shadow-2xl border border-slate-100 overflow-hidden z-[200]">
                  {promptsMau.map((p, i) => (
                    <button key={i} onClick={() => { setCustomPrompt(p.content); setShowPromptMenu(false); }} className="w-full px-4 py-3 text-left text-[10px] font-bold hover:bg-blue-50 border-b border-slate-50 last:border-0 text-slate-700 transition-colors">
                      {p.label}
                    </button>
                  ))}
                </div>
          ...
          <div className="bg-white/95 p-5 rounded-xl border border-white shadow-lg flex-1 flex flex-col min-h-0">
            <h2 className="text-[10px] font-black uppercase text-blue-600 mb-3 tracking-widest text-center">📂 Hồ sơ đính kèm</h2>
            <div onClick={() => tailieuRef.current?.click()} className="py-6 border-2 border-dashed border-blue-100 rounded-lg flex flex-col items-center justify-center cursor-pointer hover:bg-blue-50 shrink-0">
              ...
              <p className="text-[9px] font-bold text-slate-400 uppercase">Bấm để tải tệp</p>
            ...
            <div className="mt-3 space-y-2 overflow-y-auto flex-1 pr-1 custom-scrollbar">
              {selectedFiles.map((file, idx) => (
                <div key={idx} className="flex items-center gap-2 p-3 bg-blue-50/50 rounded-lg border border-white shadow-sm">
                  ...
                  <button onClick={(e) => { e.stopPropagation(); setSelectedFiles(prev => prev.filter((_, i) => i !== idx)) }} className="text-red-400 font-bold px-1">✕</button>
                </div>
              ))}
            </div>
          </div>
          <button onClick={handleAiAction} disabled={loading} className="w-full py-5 bg-blue-600 text-white rounded-xl font-black text-xs uppercase shadow-xl hover:bg-blue-700 active:scale-95 transition-all">🚀 Kích hoạt AI</button>
        </div>
        ...
        <div className="col-span-9 flex flex-col gap-4 overflow-hidden">
          <div className="bg-white/95 backdrop-blur-3xl rounded-xl border border-white flex flex-col flex-1 shadow-2xl relative overflow-hidden">
            <div className="px-8 py-4 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
              <span className="text-[10px] font-black uppercase text-blue-500/50 italic tracking-widest">Kịch bản soạn thảo số v27.0</span>
              <button onClick={() => setCustomPrompt("")} className="text-[10px] font-black text-red-400 hover:text-red-600 uppercase">Xóa trắng</button>
            </div>
            <textarea value={customPrompt} onChange={(e)=>setCustomPrompt(e.target.value)} placeholder="Chọn mẫu từ Menu và điền thông tin..." className="w-full flex-1 bg-transparent p-10 text-md outline-none resize-none font-medium text-slate-700 leading-relaxed custom-scrollbar" />
            <div className="absolute bottom-6 right-6 flex gap-3">
                <button onClick={() => window.open('https://canva.com', '_blank')} className="px-8 py-4 bg-[#8b3dff] text-white rounded-xl text-[10px] font-black uppercase shadow-lg hover:scale-105 transition-all flex items-center gap-2">🎨 Canva</button>
                <button onClick={() => {saveAs(new Blob([aiResponse]), "TaiLieu.docx");}} className="px-8 py-4 bg-emerald-600 text-white rounded-xl text-[10px] font-black uppercase shadow-lg hover:bg-emerald-500 transition-all flex items-center gap-2">📥 Tải Word</button>
            </div>
          </div>
        </div>
      </main>
      ...
      <div className={`fixed bottom-10 right-10 z-[300] transition-all duration-500 transform ${isChatOpen ? 'w-[90vw] md:w-[650px] opacity-100 translate-y-0 scale-100' : 'w-0 opacity-0 translate-y-20 scale-90 pointer-events-none'}`}>
          <div className="bg-white rounded-2xl border border-white shadow-2xl flex flex-col h-[70vh] overflow-hidden">
            <div className="p-6 bg-blue-600 text-white flex justify-between items-center shadow-lg">
                <span className="text-[11px] font-black uppercase tracking-widest">Kết quả biên soạn AI</span>
            ...
            <div className="p-10 overflow-y-auto text-[15px] leading-relaxed whitespace-pre-wrap font-medium text-slate-700 flex-1 bg-slate-50/30 custom-scrollbar">
                {loading ? "🤖 AI đang làm việc, thầy vui lòng đợi..." : aiResponse || "Hệ thống sẵn sàng."}
            </div>
          </div>
      </div>
    </div>
